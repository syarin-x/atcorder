...
// code
// ------------------------------------------------
void update_dist(vector<string> &m,vector<vector<ll>> &dist, ll t_i, ll t_j, ll s_i, ll s_j, ll x){

    bool flg = true;

    if(m[t_i][t_j] != '#') flg = false;
    ...
    if(flg){
        chmin(dist[t_i][t_j], bef + x);
    } else {
        chmin(dist[t_i][t_j], bef + 1);
    }
...
int main() {
    ...
    while(left <= right){
        ...
        vector<vector<ll>> dist(h, vector<ll>(w, INFL));
        ...
        rep(x, h * w - 1){
            rep(i,h){
                rep(j,w){
                    // ue
                    if(i - 1 >= 0){
                        update_dist(m, dist, i - 1, j, i, j, mid);
                    }
                    // shita
                    if(i + 1 < h)
                        update_dist(m, dist, i + 1, j, i ,j, mid);
                    ...
                    // migi
                    if(j + 1 < w)
                        update_dist(m,dist, i, j +1, i, j, mid);
